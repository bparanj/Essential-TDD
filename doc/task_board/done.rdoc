...
1. Copy all scripts to deploy folder

Dec 26, 12

1. Does the affiliate promote individual products or a particular vendor?
	Individual products
2. Move append_landing_page parameter(link) method to affiliate_links_helper ? Moved to LandingPage model.
3. Implement contact tab copy the code from Cabernet feedbacks using Ponymail

Dec 25, 12

1. Integrate the new UI with Cahor.
2. Integrated Devise 
3. Layout for logged in and general use.
4. Upload file to root level directory not available to public
5. Show product page grid design
6. Landing page grid design
7. New product page design

Dec 24, 12

1. Integrated CSS for the inventory tab.
2. Layout for the site.

Dec 23, 12

1. Finished SCRUM book.
2. Revised planning documents.
3. Bought EC2 screencasts.

Dec 22, 12

1. Signup for Amazon S3 account at http://aws.amazon.com 																	
2. Get Access key and secret access key (Web services account -> AWS access identifiers)  
		https://portal.aws.amazon.com/gp/aws/securityCredentials
3. Copy the secret access key and access key id. 																					
4. Create a bucket in Amazon S3 called clickplan.net																			
5. Is there a way to find out if S3 download was successful? 
...
2. Reduce duplication in instant_payment_notification_mapper
3. Refine the controller to handle all cases. If the response code is not 200, save the message for further investigation.
	--> This is done by the ipn.acknowledge else clause
    How to check the HTTP response code in ActiveMerchant? 
	--> Looking at the ActiveUtil gem, it the response status code is 200..300 then response.body is returned.
		This is done by ActiveMerchant
    IPN script should make sure to respond to the HTTP response with a 200 OK message to prevent additional attempts by PayPal to send the IPN. 
 	--> This is handled by ActiveMerchant.
4. 	Fulfillment Module

	Give link to the downloads controller /confirmation_number
	Lookup in the db and check for payment status and order status
	Redirect the user to the actual product download url that is pointing to S3 file
	Allow user to download the book when the payment is success and order status is 'fulfill'.
	Set order.status = Order::DELIVERED after they download the product


Dec 20, 12

1. Added paypal_gateway specs.
2. Added BillingResponseMapper class to simplify paypal_gateway class.
3. Added CookieJar specs
4. Added billing response mapper specs
5. File upload feature.
6. receiver_id is not used in this version.

This is part of IPN post variables:
...
Express Checkout :
transaction.details['PaymentInfo']['SellerDetails']['SecureMerchantAccountID'] gives 13 digit SLLS4V95L43LN
is the same as
transaction.details['secure_merchant_account_id']

These two values should be checked to make sure they are the same if you are using the receiver_id to credit a merchant for a transaction. 

Dec 19, 12

1. Added bounty processor specs 
2. Added specs for fraud policy
3. Added specs for instant payment notification
4. Added paypal notifications mapper specs
5. Added refund processor specs
...
3. openssl passwd -1 12345 is generating different shadow compatible password on every run. Use salt to generate
   the same password.
4. If you receive an INVALID response, then either you’ve done something wrong or the original IPN should be treated as suspicious and investigated. 
5. Use sample IPN message in paypal/paypal.rdoc to write rake task. Enhanced rake task paypal:ipn
6. Decide whether category model must be removed. Postponed to a version when Click Plan becomes a marketplace.
7.  - description != notify.params[‘item_name] 
(Use custom variable to store the id of the record that contains the pass through variable, fields stored locally). Currently PaypalService class, description contains the item_name value, change it to match the API call. _/
		 [Product name is passed in item_name.]
